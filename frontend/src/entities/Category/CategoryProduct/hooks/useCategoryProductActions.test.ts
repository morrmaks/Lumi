...
describe('useCategoryProductActions', () => {
  ...
  it('addToBasket добавляет товар и вызывает dispatch', async () => {
    const { result } = renderHook(() => useCategoryProductActions('1'))
    await act(async () => {
      const fakeEvent = {
        preventDefault: jest.fn(),
        stopPropagation: jest.fn(),
      } as unknown as React.MouseEvent<HTMLButtonElement>
      result.current.addToBasket(fakeEvent)
    })
  ...
  it('toggleWishlist добавляет и удаляет товар из wishlist', async () => {
    const { result } = renderHook(() => useCategoryProductActions('1'))
    await act(async () => {
      const fakeEvent = {
        preventDefault: jest.fn(),
      ...
      result.current.toggleWishlist(fakeEvent)
    })
  ...
  it('toggleConfigurator добавляет и удаляет компонент', async () => {
    const { result } = renderHook(() =>
      useCategoryProductActions('1', 'processor')
    )
    await act(async () => {
      const fakeEvent = {
        preventDefault: jest.fn(),
      ...
      result.current.toggleConfigurator(fakeEvent)
    })
